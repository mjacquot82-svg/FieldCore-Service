...
function renderView(view, metrics, customerMap, propertyMap) {
  ...
  if (view === 'visits') return renderVisits(propertyMap);
...
function renderTimeline(customerMap, propertyMap) {
  ...
  return `<section><h2>${customer ? `${customer.name} Timeline` : 'Customer Timeline'}</h2><button class="primary" data-nav="customers">Back to Customers</button><div class="stack timeline-stack">${events.length ? events.map((event) => `<article class="panel timeline-event"><div class="timeline-date">${event.date}</div><div><span class="badge outstanding">${event.type}</span><h3>${event.title}</h3><p>${event.detail}</p></div></article>`).join('') : '<article class="panel"><p>No timeline events found for this customer yet.</p></article>'}</div></section>`;
}
...
function renderVisits(propertyMap) {
  const customer = selectedCustomer();
  const ids = new Set(state.properties.filter((p) => !selectedCustomerId || p.customer_id === selectedCustomerId).map((p) => p.property_id));
  const visits = selectedCustomerId ? state.visits.filter((v) => ids.has(v.property_id)) : state.visits;
  return `<section><h2>${customer ? `${customer.name} Visit History` : 'Service Visits'}</h2>${customer ? '<button class="primary" data-nav="customers">Back to Customers</button>' : ''}<div class="stack">${visits.length ? visits.map((v) => `<article class="panel"><h3>${v.visit_date} · ${v.service_description}</h3><p>${propertyMap[v.property_id]?.service_address ?? 'Unknown property'}</p><p>Price ${currency(v.price)} · Status: ${v.status}</p><p>Notes: ${v.notes || 'None'}</p></article>`).join('') : '<article class="panel"><p>No visits found for this customer.</p></article>'}</div></section>`;
}
